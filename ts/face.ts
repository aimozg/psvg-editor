...
export interface AppearanceFull extends Appearance {
	skinColor: string;
	eyeColor: string;
	hairColor: string;
...
const defaultFace: AppearanceFull = getDefaultAppearance();
export function getDefaultAppearance(): AppearanceFull {
	return {
		cheekSz: 0.5,
		noseSz: 0.5,
					...
					path(noseshadow(f.s), 'nose-shadow',
						svg.dtostr([
							{c: 'M', p: [top]},
							{
								...
								svg.vlcomb([top, right], [1 / 4, 1]), right]
							}, //b
							{
								c: 'C', p: [
								svg.vlcomb([right, tip], [1, 1 / 7]), //c
								svg.vlcomb([right, tip], [1 / 4, 23 / 28]), tip]
							}, //d
							{
								c: 'C', p: [
								svg.vlcomb([tip, left], [15 / 28, 1 / 4]), top, top]
							}, //e
							{c: 'Z', p: []}
						])),
...
export function renderFace(appr: Appearance): SVGElement {
	...
	let browColor = appr.browColor || (appr.matchLash ? appr.hairColor || defaultFace.browColor : defaultFace.browColor);
